...
class VozInteligenteController extends Controller
{
    ...
    /**
     * Probar conectividad de voz inteligente
     */
    public function probarSistema(Request $request): JsonResponse
    {
        try {
            $pruebas = [];

            // Probar servicios de IA disponibles
            $pruebas['ias_disponibles'] = [
                'openai' => config('services.openai.api_key') ? true : false,
                'anthropic' => config('services.anthropic.api_key') ? true : false,
                'google_gemini' => config('services.google_gemini.api_key') ? true : false,
                'lexisnexis' => config('services.lexisnexis.api_key') ? true : false,
                'legal_ai_library' => config('services.legal_ai_library.api_key') ? true : false
            ];

            // Probar servicios de voz
            $pruebas['servicios_voz'] = [
            ...
            $pruebas['estado_sistema'] = [
                'servicio_operativo' => true,
            ...
            return response()->json([
                'exito' => true,
                'mensaje' => 'Sistema de voz inteligente operativo',
                'pruebas' => $pruebas,
                'timestamp' => now()->toISOString()
            ]);

        } catch (\Exception $e) {
            Log::error('Error probando sistema de voz', ['error' => $e->getMessage()]);

            return response()->json([
                'exito' => false,
                'error' => 'Error probando sistema de voz inteligente',
